...
function parseXMLResponse(response: string, fileHasExplicit2_2: boolean, year: number): Array<{
  ...
  while ((match = questionRegex.exec(response)) !== null) {
    ...
    if (subjectMatch && contentMatch) {
      ...
      // 强制验证：如果文件没有明确的専門2-2标识，则所有题目归类为専門1
...
请遍历全文，找出所有属于専門1和専門2-2的题目，使用 XML 格式输出。每道题必须包含 <analysis> 思维链。`;
  ...
  try {
    ...
    // 解析 XML 响应，传入文件是否有明确的専門2-2标识和年份
...
async function processFile(filePath: string): Promise<number> {
  ...
  // 预过滤：只处理包含専門1或専門2-2的文件
  const fileContentLower = content.toLowerCase();
  const hasAllowedContent =
    fileContentLower.includes("専門1") ||
    fileContentLower.includes("専門2-2") ||
    fileContentLower.includes("第2群") ||
    parsed.subjectType.includes("1") ||
    parsed.subjectType.includes("2-2");

  if (!hasAllowedContent && parsed.year >= 2017) {
    console.log(`  跳过：文件不包含専門1或専門2-2内容`);
    return 0;
  }
...
async function main() {
  console.log("═══════════════════════════════════════");
  console.log("  建筑学过去问题目切分工具 v3.0");
  console.log("  只提取 専門1 和 専門2-2 的题目");
  console.log("  新增 <analysis> 思维链 + 硬编码拦截器");
  console.log("═══════════════════════════════════════\n");
  ...
  const files = fs.readdirSync(RAW_MARKDOWN_DIR).filter((f) => f.endsWith(".md"));
